...
<body>
    ...
    <main class="admin-main">
        ...
        <?php endif; ?>
        <?php if ($errorMsg): ?>
            <p class="admin-aviso admin-aviso--error" role="alert"><?= htmlspecialchars($errorMsg, ENT_QUOTES, 'UTF-8') ?>
            </p>
        <?php endif; ?>
        ...
        <section aria-label="resumen">
            ...
            <dl class="admin-stats">
                <div class="admin-stat-card admin-stat-card--total">
                    ...
                    <!-- Nuevo -->
                    <svg class="admin-stat-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                        aria-hidden="true">
            ...
            <?php $pctPeliculas = $total > 0 ? round($peliculas / $total * 100) : 0; ?>
            <div class="admin-proporcion-barra">
                <div class="admin-proporcion-segmento--peliculas" style="width:<?= $pctPeliculas ?>%"></div>
                <div class="admin-proporcion-segmento--series" style="width:<?= 100 - $pctPeliculas ?>%"></div>
            </div>
            <div class="admin-proporcion-leyenda">
                <span>Películas <?= $pctPeliculas ?>%</span>
                <span>Series <?= 100 - $pctPeliculas ?>%</span>
            </div>
        </section>
        ...
        <section aria-label="generos mas vistos">
            ...
            <?php if ($generosMasVistos === []): ?>
                <p class="admin-vacio">Sin contenido. <a href="/admin/contenido/nuevo">Crear manual</a> o <a href="/admin/contenido/nuevo?modo=tmdb">importar de TMDB</a>.</p>
            <?php else: ?>
                <ol class="admin-ranking">
                    <?php foreach ($generosMasVistos as $i => $g): ?>
                        <li>
                            <span class="admin-ranking-pos"><?= $i + 1 ?></span>
                            <span style="flex:1"><?= htmlspecialchars((string) $g['nombre'], ENT_QUOTES, 'UTF-8') ?></span>
        ...
        <section aria-label="listado contenido">
            ...
            <?php if ($contenidoLocal === []): ?>
                <p class="perfil-vacio">Sin contenido. <a href="/admin/contenido/nuevo">Crear manual</a> o <a
                        href="/admin/contenido/nuevo?modo=tmdb">importar de TMDB</a>.</p>
            <?php else: ?>
                                ...
                                <tr>
                                        ...
                                        <?php if (!empty($item['poster_path'])): ?>
                                            <div class="poster-marco poster-marco--xs">
                                                <img class="poster-img"
                                                    src="<?= htmlspecialchars((string) $item['poster_path'], ENT_QUOTES, 'UTF-8') ?>"
                                                    alt="">
                                            </div>
                                        <?php endif; ?>
                                    </td>
                                    <td class="admin-tabla-titulo">
                                        <?= htmlspecialchars((string) ($item['titulo'] ?? ''), ENT_QUOTES, 'UTF-8') ?></td>

                                    
                                    <td>
                                        <span
                                            class="chip chip--simple <?= ($item['type'] ?? '') === 'series' ? 'chip--serie' : 'chip--pelicula' ?>">
                                            <?= ($item['type'] ?? '') === 'series' ? 'Serie' : 'Película' ?>
                                        </span>
                                    </td>


                                    <td><?= htmlspecialchars((string) ($item['anio_lanzamiento'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="admin-tabla-acciones">
                                            <a href="/admin/contenido/editar?id=<?= urlencode($id) ?>"
                                                class="boton boton--secundario boton--sm">editar</a>
                                            <form method="POST" action="/admin/contenido/eliminar"
                                                class="admin-tabla-acciones-form" onsubmit="return confirm('eliminar?');">
                                                <input type="hidden" name="_csrf"
                                                    value="<?= htmlspecialchars($csrf, ENT_QUOTES, 'UTF-8') ?>">
                                                <input type="hidden" name="id"
                                                    value="<?= htmlspecialchars($id, ENT_QUOTES, 'UTF-8') ?>">
                                                <button type="submit"
                                                    class="boton boton--secundario boton--sm">eliminar</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
